...
document.getElementById('btnPrintTransfer')?.addEventListener('click', async function() {
    ...
    if (bank === 'PANIN') {
        let jumlahBulat = Math.floor(jumlah);
        let terbilang = jumlahBulat + ' ' + supplier.currency;
        
        printContent = `
            <div style="font-family: 'Courier New', monospace; font-size: 10pt; width: 100%;">
                <div style="text-align: right; font-size: 13pt; font-weight: bold; margin-bottom: 5px;">
                    ${supplier.currency} ${jumlah.toLocaleString('en-US', {minimumFractionDigits: 2})}
                </div>
                <div style="text-align: right; font-size: 9pt; margin-bottom: 20px; border-bottom: 1px dashed #ccc; padding-bottom: 8px;">
                    ${terbilang}
                </div>
                ...
                <div style="display: flex; gap: 30px; justify-content: space-between;">
                    <div style="flex: 2;">
                        ...
                        <div>IBAN : ${supplier.account}</div>
                        <div>SWIFT: ${supplier.swift || '-'}</div>
                        <div>${supplier.country || '-'}</div>
                        <div style="height: 8px;"></div>
                        <div>${data.berita || '-'}</div>
                        <div>PAYMENT DOC NO ${data.noLoa || '-'}</div>
                        <div style="height: 8px;"></div>
